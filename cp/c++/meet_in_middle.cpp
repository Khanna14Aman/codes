...
#include<map>
using namespace std;
void solve1(int ind, vector<int>&leftarr, int long long sum, vector<int long long>&leftsum, int w)
{
	if(ind==w)
	{
...
void solve2(int ind, vector<int>&rightarr, int long long sum, map<int long long,int>&mppsum,int z)
{
	if(ind==z)
	{
		mppsum[sum]++;
...
int main()
{
	...
	int z = rightarr.size();
	int long long sum=0;
	vector<int long long>leftsum;
	map<int long long , int>mppsum;
